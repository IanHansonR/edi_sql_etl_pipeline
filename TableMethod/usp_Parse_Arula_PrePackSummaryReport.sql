/*
    Stored Procedure: usp_Parse_Arula_PrePackSummaryReport
    Purpose: Parse Arula KN and SA EDI 850 orders from EDIGatewayInbound
             and populate PrePack Summary Report tables with unique BOM definitions.

    Source: EDIGatewayInbound (filtered by CompanyCode = 'Arula', PurchaseOrderTypeCode IN ('KN', 'SA'))
    ...
    Note: Arula KN and SA POs may contain mixed BOM and non-BOM items.
          Only BOM line items are processed; POs with no BOMs are skipped.

    Arula PrePack Summary Mapping:
    - PrePackNUMBER = PurchaseOrderDetails.VendorItemNumber (parent level)
    - PrePackSKU = PurchaseOrderDetails.GTIN (parent level)
...
CREATE OR ALTER PROCEDURE dbo.usp_Parse_Arula_PrePackSummaryReport
AS
BEGIN
    SET NOCOUNT ON;
    ...
    DECLARE record_cursor CURSOR LOCAL FAST_FORWARD FOR
        ...
        WHERE CompanyCode = 'Arula'
          ...
          AND JSON_VALUE(JSONContent, '$.PurchaseOrderHeader.PurchaseOrderTypeCode') IN ('KN', 'SA')
    ...
    BEGIN
        ...
        ;WITH BOMLineItems AS (
            ...
            SELECT
                JSON_VALUE(detail.value, '$.VendorItemNumber') AS PrePackNUMBER,
                JSON_VALUE(detail.value, '$.GTIN') AS PrePackSKU,
            ...
            SELECT
                JSON_VALUE(@JSONContent, '$.PurchaseOrderHeader.PurchaseOrder.PurchaseOrderDetails.VendorItemNumber') AS PrePackNUMBER,
                JSON_VALUE(@JSONContent, '$.PurchaseOrderHeader.PurchaseOrder.PurchaseOrderDetails.GTIN') AS PrePackSKU,
        ...
        BOMComponents AS (
            SELECT
                ...
                JSON_VALUE(bom.value, '$.GTIN') AS ComponentSKU,
        ...
        SELECT
            ...
            PrePackNUMBER,
            PrePackSKU,
            NULL,  -- PrePackSTYLE (Arula: NULL)
            NULL,  -- PrePackCOLOR (Arula: NULL)
            NULL,  -- PrePackSIZE (Arula: NULL)
            NULL,  -- PrePackUPC (Arula: NULL)
            ComponentSTYLE,
            ComponentCOLOR,
            ComponentSIZE,
            NULL,  -- ComponentUPC (Arula: NULL)
            ComponentSKU,
            ComponentQTY
        FROM BOMComponents;
