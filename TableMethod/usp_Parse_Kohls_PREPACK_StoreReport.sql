/*
    Stored Procedure: usp_Parse_Kohls_PREPACK_StoreReport
    Purpose: Parse Kohl's PREPACK and COMPOUND PREPACK EDI 850 orders from EDIGatewayInbound and populate Store Report tables
    ...
    Detail rows are aggregated by StoreNumber with StoreQty = SUM of all line item quantities for that store.
*/

CREATE OR ALTER PROCEDURE dbo.usp_Parse_Kohls_PREPACK_StoreReport
AS
BEGIN
    SET NOCOUNT ON;
